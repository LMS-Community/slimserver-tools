...
my @defaultStringfiles = qw/strings.iss/;
my %strings;
my $debug = 1;
...
while (defined (my $sltFile = readdir(DIR))) {
	...
	open(MYSTRINGS, "<", $sltFile) or (warn "Couldn't open $sltFile for reading: $!\n" && next);
	binmode MYSTRINGS;
...
find sub {
	...
	if (grep { $stringsFile =~ /$_$/ } @defaultStringfiles) {
		...
		if (-w $stringsFile) {
			...
			open(STRINGS, ">", $tmpFile) or die "Couldn't open $tmpFile for writing: $!\n";
			binmode STRINGS;
...
sub mergeCustomStrings {
	...
	while ($stringCopy =~ /(^\S+.*?)(^\s*\n|\z)/gsmi) {
		...
		$stringsToTranslate =~ /^[a-z]{2}\.(\S+)=/m;
		my $stringName = $1;

		# no white space allowed in token
		if (defined $strings{$stringName}) {
			foreach my $language (keys %{$strings{$stringName}}) {
				# try to replace the translation...
				if ($stringsToTranslate !~ s/^($language\.$stringName)=.+?$/$1=$strings{$stringName}->{$language}/ism) {
					# ... or simply add it
					$stringsToTranslate .= sprintf("%s.$stringName=%s\n", lc($language), $strings{$stringName}->{$language});
				}
			}
#			$stringsToTranslate = sortStrings($stringsToTranslate);

			if ($stringsToTranslate !~ /\;/) {
				$$originalStrings =~ s/(^[a-z]{2}\.$stringName=.*?)(^\s*\n|\z)/$stringsToTranslate\n/smi;
			}
			delete $strings{$stringName};
		}
		elsif ($debug && $stringName !~ /^\;/ && !defined $strings{$stringName}) {
